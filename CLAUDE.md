...
**Tank-tank collision (`Scene.svelte`, `Tank.svelte`):** `Scene.svelte` provides a `tankBodies: {x, z}[]` array via `setContext('tankBodies', tankBodies)`. Each `Tank` creates its own `ownBody` entry (`{x: spawnX, z: spawnZ}`), pushes it into `tankBodies` at component init, and removes it in `onDestroy`. After every physics frame the entry is updated to the tank's resolved position. The collision pass runs after tree-trunk collision: for each entry that is not `ownBody`, if `distSq < (TANK_RADIUS * 2)²`, push out to the contact boundary and zero speed if moving toward the other tank. Both tanks resolve independently each frame, so the repulsion is mutual even without explicit two-body integration. The `{x: spawnX, z: spawnZ}` initialisation ensures the slot is correctly populated before the first `useTask` frame runs.
