...
**Tank spawn positions (`Scene.svelte`, `Tank.svelte`):** `initGame()` computes spawn positions *before* terrain or tree generation so trees can avoid them. Tanks sit on a circle at `SPAWN_RADIUS = (WORLD_SIZE/2) * 0.8` (300 units), divided into `TANK_COUNT` equal arcs from a random `baseAngle` for per-game variety. Each position gets up to 10% radial jitter (`±SPAWN_RADIUS * 0.1`). Positions are stored in `spawnPositions` (`$state`). The tree grid skips any candidate within `SPAWN_CLEAR = 40` units of any spawn. Scene passes `spawnX/spawnZ` as props to Tank; `initGame()` calls `tankRef.reset(x, z)` with the new position directly (not relying on prop propagation timing). Tank's `reset(sx, sz)` snaps to the given coords; `onMount` snaps to the `spawnX/spawnZ` props (set before Tank mounts since `initGame()` is synchronous).
