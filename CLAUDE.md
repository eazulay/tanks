...
**Splash component (`Splash.svelte`):** Props: `x`, `z`, `onremove`. Manages 3 rings with staggered delays (0, 0.4, 0.8 s); each expands from radius 0 to 6 over 2 s while fading from opacity 0.7 to 0. Rendered as `<T.Mesh><T.RingGeometry>` in Svelte markup (reactive `$state<Ring[]>`). Ring material uses `color="#ffffff" depthWrite={false}` — white so blending with the water gives a natural cyan tint; `depthWrite={false}` prevents the ring from punching a transparent hole through the water plane. Calls `onremove()` once all rings have completed. Used by `Shell.svelte`, `Explosion.svelte`, and `Tank.svelte` (wake waves).
