...
**Feedback form:** Optional, gated entirely by env var presence — see README.md's "Feedback form" section for the actual env vars and schema DDL (not duplicated here). `src/lib/server/env.ts` exports `isFeedbackConfigured()` (presence-only check, no DB round-trip — a transient DB outage must not hide the feature) and `getEnv()`. `Footer.svelte` fetches `GET /api/feedback` on mount and only renders the "Feedback" trigger button, the `<FeedbackModal>` mount, and an added Privacy Policy paragraph when `enabled` is true — an unconfigured clone of the repo (no `.env` DB vars) shows none of it, and `POST /api/feedback` also 404s in that case. `src/lib/server/db.ts` deliberately does **not** create or migrate the `feedback` table — created manually once, app's DB credentials are least-privilege (`SELECT`/`INSERT`/`UPDATE` only, no DDL) rather than self-provisioning via `CREATE TABLE IF NOT EXISTS` (which only ever helps once anyway and can't handle later schema changes). `src/lib/server/rateLimit.ts` is an in-memory per-IP limiter (~5/10 min), process-local by design (works because `adapter-node` is one long-lived process, not serverless). Honeypot field is named `website` everywhere — DOM `name` attribute, Svelte variable, JSON key, all the same — deliberately mundane so it isn't recognizable as a trap to either a naive scanning bot or someone replaying a captured request; its hiding CSS class is `.website-field`, not something like `.visually-hidden`, since well-known hidden-field utility class names are themselves a signal sophisticated bots watch for. Honeypot-triggered and rate-limited requests both return a fake `201` success (not an error) to avoid tipping off bots; real validation failures (empty message, malformed email) return real `400`s. IP address is stored raw (`ip_address` column), not hashed — GDPR's "legitimate interest" basis covers spam/abuse prevention (the same basis tools like Wordfence rely on for IP logging/blocking), and raw storage enables actually blocking a specific IP later rather than only noticing a cluster after the fact; disclosed in the Privacy Policy paragraph. No email digest or admin/moderation view exists in this repo — a separate, external PHP app reads the same `feedback` table on a cron job for that; `approved`/`digested_at` columns are written as defaults only and never read back here. Known gap, not yet resolved: `event.getClientAddress()` needs adapter-node's `ADDRESS_HEADER`/`XFF_DEPTH` configured correctly behind Cloudflare + cPanel's proxy for rate limiting to key on real visitor IPs rather than one upstream IP.

**Debug spectator camera (`Scene.svelte`/`+page.svelte`):** A hidden dev-only camera for chasing down "AI tank alive but nowhere to be found" bugs, gated behind `DEBUG_CAM_ENABLED = import.meta.env.VITE_DEBUG_CAM === '1'`. All of its logic (key handling, camera follow, on-screen banner) lives inside `if (DEBUG_CAM_ENABLED)` blocks so a normal `npm run build` (with the env var unset) dead-code-eliminates it entirely — verified by grepping `.svelte-kit/output` after a flag-off build. This matters for multiplayer fairness: the feature must never ship in a build real players connect to. Activation steps and controls are intentionally not documented here (public repo) — see `DEBUG_CAMERA.local.md` (gitignored) instead.
